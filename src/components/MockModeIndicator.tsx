...
export function MockModeIndicator() {
  ...
  return (
    <div
      role="status"
      className="w-full bg-[var(--color-warning)] px-4 py-2 text-center text-sm font-medium text-[var(--color-warning-foreground)]"
    >
      <p className="font-semibold tracking-wide">
        {contextLabel} — some details still pending confirmation · noindex
      </p>
      <p className="mt-0.5 text-xs font-normal opacity-90">
        Not every address, timetable, or fee on this preview is final. This banner stays until remaining
        content is verified.
      </p>
    </div>
  );

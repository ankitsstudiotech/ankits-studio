...
export type LocationTrialCtaProps = {
  branchName: string;
  title?: string;
  body?: string;
  ctaLabel?: string;
...
export function LocationTrialCta({
  branchName,
  title = "Visit this branch",
  body,
  ctaLabel = "Book a trial",
  ctaHref = "/trial",
}: LocationTrialCtaProps) {
  return (
    <Section id="trial" eyebrow="Trial" title="Book a visit">
      <Card className="flex flex-col items-start gap-5 bg-accent-soft/80 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0 max-w-xl">
          ...
          <Body>
            {body ??
              `Try a class at ${branchName}. Contact actions stay disabled until this branch is verified.`}
          </Body>
        </div>
        <Link
          href={ctaHref}
